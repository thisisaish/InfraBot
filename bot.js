...
const { ActivityTypes, CardFactory, BotFrameworkAdapter,MemoryStorage } = require('botbuilder');
...
const { WeatherCard } = require('./dialogs/weather');
const { StockCard } = require('./dialogs/stock');
const { RestaurantCard } = require('./dialogs/restaurant');
const { SportsCard } = require('./dialogs/sports');
const { ReminderCard } = require('./dialogs/tweet');
const { GreetingDialog } = require('./dialogs/greeting');
const { SearchCard } = require('./dialogs/search');
...
const WEATHER_INTENT = 'Weather_GetForecast';
const STOCK_INTENT = 'Stock';
const TWITTER_INTENT = 'Twitter';
const SEARCH_INTENT = 'search';
const SPORTIVE_INTENT = 'Sport';
const CALENDARADD_INTENT = 'Calendar_Add';
const FOOD_INTENT = 'restaurant';
const SEARCH_ENTITY = 'seagull';
const sport = 'tennis';
                    ...
                    case DialogTurnStatus.empty:
                        ...
                        switch (topIntent) {
                            case GREETING_INTENT:
                                ...
                                //     apiKey: 'AtegvoGKeKupY05YsYLix6pJQ3J1nI3IVKMEcOWrlxAUUkwia_6Fwgp8kUiWPNY_', // for Mapquest, OpenCage, Google Premier
                            ...
                            case WEATHER_INTENT: 
                                weather.find({search: USER_LOCATION_ENTITIES , degreeType: 'F'}, function(err, result) {
                                    if(err) console.log(err);
                               
                                    console.log(JSON.stringify(result, null, 2));
                                });
                                const weatherCard = CardFactory.adaptiveCard(WeatherCard);
                                await context.sendActivity({ attachments: [weatherCard] });
                                break;

                            case SEARCH_INTENT:
                                let credentials = new CognitiveServicesCredentials('409b04fdf82e4e1bb3f3154ed0bd3553');
                                let webSearchApiClient = new WebSearchAPIClient(credentials);
                                webSearchApiClient.web.search(SEARCH_ENTITY).then((result) => {
                                ...
                                const searchCard = CardFactory.adaptiveCard(SearchCard);
                                await context.sendActivity({ attachments: [searchCard]});
                                break;

                            case CALENDARADD_INTENT:
                                await dc.context.sendActivity(`Event created`);
                                const reminderCard = CardFactory.adaptiveCard(ReminderCard);
                                await context.sendActivity({ attachments: [reminderCard]});
                                break;
                                
                            case STOCK_INTENT:
                                // await dc.context.sendActivity(`CalenderAdd intent found, entities included: ${JSON.stringify(results.entities)}`);
                                const GDAX = require("gdax");
                                ...
                                // publicClient.getProducts(callback);
                                const stockCard = CardFactory.adaptiveCard(StockCard);
                                await context.sendActivity({ attachments: [stockCard]});
                                break;


                            case TWITTER_INTENT:
                                await dc.context.sendActivity('Here are your Twitter updates!');
                                 var client = new Twitter({
                                    consumer_key: 'uSKdt61Pe9mAnf9JqvqVUxOvQ',
                                    consumer_secret: 'XHt5simg238F9XYXbJEHkzn63oriMZvMxTgtpqIj3F3j1MSv93',
                                    access_token_key: '727654459946500096-Dk2szcnACiuGLKvluYQCxPyciD1Wwqx',
                                    access_token_secret: 'YEsYWsp53buEfeJ95vgJH6tNfyfBleuAraITIK3BQ5CCK'
                                  });
                                  client.post('trends/place', {status: 'I Love Twitter'},  function(error, tweet, response) {
                            ...
                            case FOOD_INTENT:
                                await context.sendActivity(`Here are dishes from the top restaurants`);
                                const restaurantCard = CardFactory.adaptiveCard(RestaurantCard);
                                await context.sendActivity({ attachments: [restaurantCard]});
                                var client = zomato.createClient({
                                    userKey: '08615f02e05e9bc6e1ac33c63f17b012', //as obtained from [Zomato API](https://developers.zomato.com/apis)
                                });
                                client.getRestaurant({
                                    res_id:"18836501" // id of restaurant whose details are requested
                                    }, function(err, result){
                                        if(!err){
                            ...
                            case SPORTIVE_INTENT:
                            api.getAllMatches(sport,
                                function(err,matches){ 
                                    if (err) { console.log(err.message); 
                                    } else { (matches).forEach(element =>{
                                            console.log(element.team1+" "+element.team2+"\n "+element.score+"  "+element.currentStatus);
                                    }) 
                                } 
                                });
                                const sportsCard = CardFactory.adaptiveCard(SportsCard);
                                await context.sendActivity({ attachments: [sportsCard] });
                                break;
